"""Waveform Manager module for TEM waveform analysis.

This module provides functionality for waveform manager.
"""
...
class WaveformManager:
    ...
    def analyze_single_waveform(self, wave_func, width, name, save_data=True):
        ...
        self.visualizer.generate_report(
            result, f"{name.replace(' ', '_')}_{width*1e3:.1f}ms_report.txt"
        )
    ...
    def compare_waveforms(self, wave_funcs, names, width):
        ...
        for wave_func, name in zip(wave_funcs, names):
            ...
            # 收集结果
            result = self.analyze_single_waveform(
                wave_func, width, name, save_data=False
            )
            results.append(result)
        ...
        return {"width": width, "waveforms": names, "results": results}
    ...
    def run_comprehensive_analysis(self):
        ...
        if self.generator.has_simpeg:
            ...
            wave_names.extend(
                ["SimPEG Trapezoid", "SimPEG Differential Pulse", "SimPEG Step-O"]
            )
